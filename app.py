...
def _processing_md(elapsed: float) -> str:
    return (f"⏳ **Processing… {elapsed:0.0f}s** — gathering evidence, scoring, "
...
def _metrics_md(state: dict) -> str:
    ...
    warn = ""
    if state.get("errors"):
        warn = f"\n\n**Warnings ({len(state['errors'])}):** " + "; ".join(state["errors"][:3])
    return (
        f"### Run metrics\n"
...
.mt-card { background:#ffffff !important; border:1px solid #e2e8f0 !important;
...
#mt-upload .center { min-height:96px !important; padding-top:10px !important; padding-bottom:10px !important; }
...
#mt-dl { background:#ffffff; border:1px solid #cbd5e1; border-radius:14px; padding:6px;
            ...
            with gr.Column():
                with gr.Group(elem_classes="mt-card"):
                    context = gr.Textbox(
                        ...
                        lines=4,
                        placeholder="e.g. UK retail bank; real-time orchestration on "
                                    "Snowflake; data must stay in the EU; SOC 2 required.",
                    )
