...
# JUDGE (consistency) = OpenAI. A different provider on purpose, so the judge
# pass measures genuine cross-provider disagreement.
# --------------------------------------------------------------------------- #

PRIMARY_MODEL = os.getenv("MARTECH_PRIMARY_MODEL", "claude-sonnet-4-5")
JUDGE_MODEL = os.getenv("MARTECH_JUDGE_MODEL", "gpt-4o-mini")
...
# evidence behind it. This stops a public-source agent from awarding a 5 off a
# vendor marketing page alone.
# --------------------------------------------------------------------------- #
...
# Tier 1 alone caps at 3: a self-asserted claim cannot earn "strong" or
# "best-in-class" without independent corroboration.
...
# Compatibility shim for older callers. The new workflow does not run
# must-have gates or eliminate vendors.
DEFAULT_GATES: list = []
